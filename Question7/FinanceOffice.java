...
public class FinanceOffice {
		...
		for(Payer p: payers) {
			...
			if(p.getName().equals(name)) {
				...
				if(p instanceof Student) {
					...
					throw new NegativeSalaryException("An employee cannot be overpaid by " + (amount-(-p.getDebt())) + " yuans!");
				} else if(p instanceof Employee && (p instanceof FacultyMember == false) && amount <= (-p.getDebt())) {
					p.pay(amount);
					return;
				} else {
	...
	public static void testFinanceOffice() {
		...
		try {
			...
			System.out.println(n.getMessage().equals("An employee cannot be overpaid by 10000 yuans!"));
		}
