...
public class Employee extends Person implements Payer {
	...
	public void pay(int amount) throws NegativeSalaryException {
		...
		if(this.getDebt() > 0) {
			...
			this.setDebt(this.getDebt() - amount);
			throw new NegativeSalaryException("An employee cannot be overpaid by " + (amount-(-this.getDebt())) + " yuans!");
		} else {
			return;
		}
	}
		public static void testEmployee() {
			...
			try {
				...
				try {
					...
					e.pay(50000);	//change the amount of debt using pay method.
					...
					System.out.println(n.getMessage().equals("An employee cannot be overpaid by 20000 yuans!"));
				}
			} catch(NegativeSalaryException n) {
